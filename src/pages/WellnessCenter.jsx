...
const WellnessCenter = () => {
  ...
  // Sync state changes with HTML5 Audio element properties
  useEffect(() => {
    if (!audioRef.current) return;
    const audio = audioRef.current;
    
    if (playingSoundId) {
      const sound = ambientSounds.find(s => s.id === playingSoundId);
      if (sound) {
        setAudioError(null);
        audio.volume = volume;
        audio.loop = loop;
    ...
    <div className="min-h-screen bg-brand-bg text-brand-dark flex flex-col transition-colors duration-200">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 md:p-8 overflow-y-auto max-w-7xl mx-auto w-full space-y-12">
          ...
          {/* ========================================================
              NEW SECTION: FOCUS & RELAXATION SOUNDS
              ======================================================== */}
          <section className="space-y-4 text-left scroll-reveal reveal-up">
            <div className="flex items-center space-x-2">
              <Music className="h-5 w-5 text-brand-secondary" />
              <h2 className="text-base font-black text-brand-dark uppercase tracking-wider">Focus & Relaxation Sounds</h2>
            </div>
            ...
            <div className={`glass-panel p-5 rounded-3xl border space-y-4 transition-all duration-500 ${playingSoundId ? 'border-brand-secondary/25 shadow-lg shadow-brand-secondary/5' : 'border-brand-secondary/15'}`}>
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                {/* Controls and Audio Progress */}
                <div className="flex items-center space-x-4 w-full sm:w-auto justify-between sm:justify-end flex-1">
                  {/* Play/Pause Button */}
                  <button
                    onClick={() => {
                      if (!playingSoundId) {
                        setPlayingSoundId(ambientSounds[0].id);
                      } else {
                        setPlayingSoundId(null);
                      }
                    }}
                  ...
                  {/* Progress Bar */}
                  <div className="flex-1 max-w-[200px] flex items-center space-x-2">
                    <span className="text-[9px] font-bold text-brand-secondary tabular-nums">
                      {playingSoundId ? formatTime(Math.round(currentTime)) : '00:00'}
                    </span>
                    ...
                    <span className="text-[9px] font-bold text-brand-secondary tabular-nums">
                      {playingSoundId && duration ? formatTime(Math.round(duration)) : '--:--'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <input
                      ...
                      className="w-20 accent-brand-secondary h-1.5 bg-slate-200 dark:bg-slate-850 rounded-lg appearance-none cursor-pointer"
                    />
                  </div>
                </div>
              </div>
              ...
              {audioError && (
                <div className="p-3 bg-rose-500/10 border border-rose-500/20 text-rose-600 rounded-2xl text-[10px] font-bold animate-in fade-in duration-150 flex items-center space-x-2">
              ...
              {ambientSounds.map((sound, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 pt-3 border-t border-brand-secondary/10 space-y-3">
                      {/* Volume Control */}
                      <div className="flex items-center space-x-2">
                        <Volume2 className="h-3.5 w-3.5 text-brand-secondary flex-shrink-0" />
                        <input
                          type="range"
                          min="0"
                          max="1"
                          step="0.01"
                          value={isCurrent ? volume : 0.5}
                          onChange={(e) => {
                          ...
                          className="w-full accent-brand-secondary h-1 bg-slate-200 dark:bg-slate-850 rounded-lg appearance-none cursor-pointer"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        {/* Duration Label */}
                        <span className="text-[10px] font-extrabold text-brand-secondary">
                          {isCurrent && duration ? formatTime(Math.round(duration)) : 'Looping'}
                        </span>
                      </div>
                    </div>
                  </div>
